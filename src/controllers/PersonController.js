...
class PersonController {
    async createPerson(req, res) {
        ...
        await database.query('UPDATE person SET education = ($2)::integer WHERE id = ($1)::integer;', [ newPerson.rows[0].id, newEducation.rows[0].id ]);
        await database.query('UPDATE person SET fullName = ($2)::integer WHERE id = ($1)::integer;', [ newPerson.rows[0].id, newFullName.rows[0].id ]);
